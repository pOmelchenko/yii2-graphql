...
#### Инлайновые определения запросов

Если создавание отдельного класса `GraphQLQuery` избыточно, поле можно описать напрямую в конфигурации — обычным массивом, совместимым с `webonyx/graphql-php`. Укажите тип, аргументы и резолвер (замыкание или `[$class, 'method']`). `GraphQL::type()` теперь можно вызывать напрямую даже до инициализации `Yii::$app`, поэтому достаточно передать готовый `Type`:
...
Такой формат удобен для простых полей и повторно используемых резолверов. При сложной логике по‑прежнему можно создавать отдельные `GraphQLQuery`/`GraphQLMutation`, где инкапсулированы `type()`, `args()`, `resolve()` и `rules()`.
...
При включённых опциях выбранная mutation должна использовать POST и как исходный транспортный HTTP-метод, и как
эффективный метод Yii, а также иметь настроенный callback `checkAccess`. Параметр Yii `_method` и заголовок
`X-Http-Method-Override` не могут повысить non-POST транспорт до POST или понизить POST до safe-метода, сохранив
выполнение `requirePostForMutations`. Если приложение намеренно доверяет method override, задайте этому флагу `false`
и проверяйте прокси и override до запуска `GraphQLAction`.
...
Файл `.gitlab-ci.yml` содержит job `publish_package`, который добавляется в пайплайн только для тегов строгого формата `MAJOR.MINOR.PATCH` без префикса `v`, например `0.17.1`. Job публикует версию в приватном GitLab Composer Registry через API и `CI_JOB_TOKEN`. Если в настройках репозитория (Settings → Repository → Mirroring repositories) включена опция **Trigger pipelines when updates are mirrored**, GitLab создаст пайплайн после синхронизации тега, но `publish_package` необходимо запустить вручную.
...
3. Создайте и запушьте тег формата `MAJOR.MINOR.PATCH` без префикса `v`, например `0.17.1`.
